                                ...
                                @if($orders -> count != 0)
                                    <div class="row p-2 bg-white border rounded items mt-2">
                                        <div class="col-md-3 mt-1">
                                            <img class="card-img-top object-fit-cover border rounded" style = "height : {{count($orders) * 1}}rem; width : 100%" src="{{asset('assets/img/logo_small.png')}}" alt="Splurge Bin">
                                        </div>
                                        <div class="col-md-6 mt-1">
                                            <h6>Accept all : </h6>
                                            @foreach($orders as $order)
                                                @if($order -> status == 0)
                                                    <div class="mt-1 mb-1 spec-1 productName">{{$order -> product -> name}} by {{$order -> customer -> name}}</div>
                                                @endif
                                            @endforeach
                                        </div>
                                        <div class="align-items-center align-content-center col-md-3 border-left mt-1">
                                            <div class="d-flex flex-row align-items-center">
                                                <h4 class="mr-1 productPrice"></h4>
                                            </div>
                                        </div>
                                        <div class="d-flex flex-column mt-4">
                                            <a class = "btn btn-primary btn-sm" href = "{{route('orders.acceptall')}}">
                                                Accept all
                                            </a>
                                        </div>
                                    </div>
                                    ...
                                    @foreach($orders as $order)
                                        <div class="row p-2 bg-white border rounded items cat-{{$order -> product -> category_id}} mt-2">
                                            ...
                                            <div class="col-md-6 mt-1">
                                                ...
                                                <div class="mt-1 mb-1 spec-1"><h5 class = "sellerName">Ordered By : {{$order -> customer -> name}}</h5></div>
                                            </div>
                                            <div class="align-items-center align-content-center col-md-3 border-left mt-1">
                                                <div class="d-flex flex-row align-items-center">
                                                ...
                                                <div class="d-flex flex-column mt-4">
                                                    <a href = "{{route('seller.order.view', $order -> id)}}" class="btn btn-primary btn-sm">
                                                        Details
                                                    </a>
                                                    @if ($order -> status == 0)
                                                        <br>
                                                        <button type = "button" class = "btn btn-outline-primary btn-sm" onclick = "acceptOrder({{$order -> id}})">
                                                            Take Order <i class="bi bi-check-lg"></i>
                                                        </button>
                                                    @endif
                                                </div>
                                            </div>
